...
date: "2026-03-27"
tags: ["nextjs", "socket-io", "stripe", "redis", "supabase", "prisma"]
published: true
---

## What is ArgenGaming?

ArgenGaming is a digital marketplace for gaming assets. Think game currencies, accounts, rare items, and boosting services. Buyers browse listings, pay through whatever method works for them (credit card, crypto, or regional payment options), and sellers deliver the goods. The platform handles escrow, disputes, verification, and everything in between.

I originally built the whole thing on **Next.js 14** with **TypeScript**, backed by **PostgreSQL** through **Prisma ORM**. The Prisma schema alone is 1,752 lines across 49+ models. The app has 289+ API routes, real-time messaging with **Socket.io**, and supports three languages out of the box.

Recently I had to migrate to **Next.js 15** because of **React2Shell** (CVE-2025-55182), the CVSS 10.0 RCE vulnerability that hit React Server Components in December 2025. An unsafe deserialization flaw in the RSC Flight data-handling logic meant that an attacker could execute arbitrary server-side code with no authentication required. For a marketplace handling real money and user data, that's not something you sit on. The patch only covered React 19.x, so upgrading Next.js was the only path forward.
...
This was one of the harder parts. ArgenGaming serves users across Latin America, so a single payment provider wasn't going to cut it. I integrated four gateways:
...
### Multi-Tier User Verification

Users go through progressive verification levels:

1. **Email verification** (basic access)
2. **Phone verification** via **Twilio** SMS
3. **Document verification** (government ID upload + selfie holding the ID)

Higher verification unlocks higher transaction limits. An unverified account can browse but can't sell. A fully verified account gets access to higher payout limits and priority support.

## The Loyalty System: Argenpuntos

I wanted to give users a reason to stick around. Argenpuntos is a points system where users earn rewards for purchases, referrals, and platform activity. Points can be redeemed for discounts on future transactions.
...
ArgenGaming supports **English, Spanish, and Portuguese** through **next-intl**. I set this up from the start rather than retrofitting it later, which saved a lot of pain.

Every user-facing string goes through the translation system. Prices display in the user's local currency. Dates follow locale conventions. The URL structure uses language prefixes (`/en/`, `/es/`, `/pt/`).
...
- **Frontend:** Next.js 15, React 18, TypeScript, Tailwind, Zustand, Framer Motion
- **Backend:** PostgreSQL, Prisma ORM (49+ models), NextAuth.js
- **Real-time:** Socket.io with Redis adapter (Upstash)
- **Payments:** Stripe, Triple A, Coinbase Commerce, KamiPay
- **Storage:** Supabase
- **Communication:** Intercom, Mailgun, Twilio, Discord webhooks
- **Security:** IPQS fraud detection, multi-document verification
